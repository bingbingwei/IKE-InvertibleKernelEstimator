...
class DataGenerator(Dataset):
    ...
    def __init__(self, conf, gan):
        ...
        # Read input image
        self.input_image = read_image(conf.input_image_path, conf.read_mode) / 255.
        ...
        # Define Channel
        self.channel = 3 if conf.read_mode == 'RGB' else 1
        self.scale_factor = conf.scale_factor

        if conf.residual_supervised:
            import torch.nn as nn
            r_input_image_lr = read_image(conf.ressup_image_path+'LR.png', conf.read_mode) / 255.
            r_input_image_hr = read_image(conf.ressup_image_path+'HR.png', conf.read_mode) / 255.
            r_input_image_lr = im2tensor(r_input_image_lr)
            r_input_image_hr = im2tensor(r_input_image_hr)

            upsampler = nn.Upsample(scale_factor=1/conf.scale_factor, mode='bicubic')
            r_input_bic = upsampler(r_input_image_lr)
            self.r_input_image = r_input_image_hr - r_input_bic

        self.residual_supervised = conf.residual_supervised
    ...
    def __getitem__(self, idx):
        ...
        if self.residual_supervised:
            r_in = self.next_r_crop(idx=idx)

            return g_in, d_in, r_in
        return g_in, d_in

    def get_vis_crop(self, top, left):
        g_in = self.vis_crop(for_g=True, top=top, left=left)
        d_in = self.vis_crop(for_g=False, top=top, left=left)
        return g_in, d_in
    ...
    def next_r_crop(self, idx):
        size = self.g_input_shape /self.scale_factor
        top, left = self.get_top_left(size, True, idx)
        crop_im = self.r_input_image[:,:,int(top/self.scale_factor):int(top/self.scale_factor + size), int(left/self.scale_factor):int(left/self.scale_factor + size)]
        return crop_im


    def vis_crop(self, for_g, top, left):
        """Return a crop according to the pre-determined list of indices. Noise is added to crops for D"""
        size = self.g_input_shape if for_g else self.d_input_shape
        crop_im = self.input_image[top:top + size, left:left + size, :]
        if not for_g:  # Add noise to the image for d
    ...
    def make_list_of_crop_indices(self, conf):
        # TODO
        iterations = conf.max_iters+conf.z_iters
...
class DataGenerator_supervised(Dataset):
    def __init__(self, conf, gan):
        # Default shapes
        self.g_input_shape = conf.input_crop_size
        self.d_input_shape = gan.G.output_size
        self.d_output_shape = self.d_input_shape - gan.D.forward_shave

        # Read input image
        self.input_image = read_image(conf.input_image_path, conf.read_mode) / 255.

        self.label_image = read_image(conf.eval_image_path, conf.read_mode) / 255.

        self.in_rows, self.in_cols = self.label_image.shape[0:2]

        # Create prob map for choosing the crop
        self.crop_indices_for_g = self.make_list_of_crop_indices(conf=conf)
        # Define Channel
        self.channel = 3 if conf.read_mode == 'RGB' else 1
        self.scale_factor = conf.scale_factor
    ...
    def __getitem__(self, idx):
        """Get a crop for both G and D """
        g_in, d_in = self.next_crop(idx=idx)
        return g_in, d_in

    def next_crop(self, idx):
        """Return a crop according to the pre-determined list of indices. Noise is added to crops for D"""
        size_big = self.g_input_shape
        size_sml = self.d_input_shape
        top, left = self.get_top_left(size_sml, idx)
        crop_im_sml = self.label_image[top:top + size_sml, left:left + size_sml, :]
        crop_im_big = self.input_image[int(top/self.scale_factor):int(top/self.scale_factor) + size_big, int(left/self.scale_factor):int(left/self.scale_factor) + size_big, :]
        return im2tensor(crop_im_big), im2tensor(crop_im_sml)
    ...
    def create_prob_maps(self, scale_factor):
        # Create loss maps for input image and downscaled one
        loss_map_big = create_gradient_map(self.input_image)
        loss_map_sml = create_gradient_map(imresize(im=self.label_image, scale_factor=scale_factor, kernel='cubic'))

        # Create corresponding probability maps
        prob_map_big = create_probability_map(loss_map_big, self.d_input_shape)
        prob_map_sml = create_probability_map(nn_interpolation(loss_map_sml, 1), self.g_input_shape)
        return prob_map_big, prob_map_sml
    ...
    def get_top_left(self, size, idx):
        """Translate the center of the index of the crop to it's corresponding top-left"""
        center = self.crop_indices_for_g[idx]
        row, col = int(center / self.in_cols), center % self.in_cols
        top, left = min(max(0, row - size // 2), self.in_rows - size), min(max(0, col - size // 2), self.in_cols - size)
