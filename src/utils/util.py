...
def write_image(img, img_path):
    img = Image.fromarray(tensor2im(img))
    img.save(img_path)
...
def tensor2tb_log(im_t):
    ...
    im_np = np.clip(np.round((move2cpu(im_t) + 1) / 2.0 * 255.0), 0, 255)
...
def rgb2gray_tensor(im):
    mask = torch.Tensor([0.299, 0.587, 0.114]).cuda()
    return im*mask[None, :, None, None]

def hsv2rgb(im):
    im = Image.fromarray(im, 'HSV').convert('RGB')
    return np.array(im)
...
def post_process_k(k, n):
    ...
    k = move2cpu(k)
    # Zeroize negligible values n=trheshold
...
def check_key_in_dict(keys, dict):
    for item in keys:
        if item in dict:
            return True
    return False
...
def down_up_scale(src, mode='bicubic', scale_factor=0.5):
    downsampler = nn.Upsample(scale_factor=scale_factor, mode=mode, align_corners=True)
    upsampler = nn.Upsample(scale_factor=1/scale_factor, mode=mode, align_corners=True)
    dst_LR = downsampler(src)
    dst_HR = upsampler(dst_LR)
    return dst_LR, dst_HR


def save_model(gan, path):
    if not os.path.exists(path):
        os.makedirs(path)
    torch.save(gan.G, path+'netG.pth')
    torch.save(gan.U, path+'netU.pth')
    return

def load_model(gan, path):
    gan.G = torch.load(path+'netG.pth')
    gan.U = torch.load(path+'netU.pth')
    return
