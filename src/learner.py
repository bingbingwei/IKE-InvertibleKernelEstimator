class Learner:
    ...
    lambda_energy_end = 5e-6
    lambda_energy_start = 4
    ...
    def __init__(self, conf):
        ...
        self.conf = conf
        if conf.use_res:
            self.bic_loss_to_start_change = 1
    ...
    def update(self, iteration, gan):
        ...
        # Once similar to bicubic is satisfied, consider inserting other constraints
            ...
            if self.insert_constraints and gan.lambda_bicubic < 5e-3:
                ...
                gan.lambda_energy = self.lambda_energy_start
                gan.start_image_discriminate = True
                gan.start_z_up_train = True
                self.insert_constraints = False

        if (iteration+1)%500 == 0 and gan.gd_ratio > 2:
            gan.gd_ratio -= 2
